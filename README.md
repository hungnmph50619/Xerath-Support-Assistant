...
**Giới hạn dung lượng:** tối đa 250 ảnh đã chọn / 200 MB tổng dung lượng tệp `sample-*` trong thư mục `%LOCALAPPDATA%\\XerathSupportAssistant\\minimap-labeled-samples-v1.8`. Đạt giới hạn sẽ ngừng lưu, không tự xóa ảnh đã gắn nhãn. Bộ xem giữ tối đa một ảnh JPEG gần nhất trong RAM: ảnh sẽ bị thay thế khi có kết quả phân tích ảnh tiếp theo, hoặc giải phóng khi bạn bấm Dừng/đóng cửa sổ. **Để lưu ảnh cần chủ động xem và chọn trước khi ảnh kế tiếp thay thế hoặc phiên dừng; bản này chưa có màn hình duyệt các ảnh chưa chọn sau trận.** Nút “Xóa toàn bộ mẫu V1.8” yêu cầu xác nhận và chỉ xóa các tệp JPG/JSON của thư mục riêng này, không xóa dữ liệu ảnh phiên bản cũ. Gemini có thể giữ dữ liệu theo chính sách riêng; xóa tệp cục bộ không xóa dữ liệu ở dịch vụ ngoài.

**Cách thử trên Windows:** cập nhật từ `main`, chạy `.\\RUN_WINDOWS.cmd`, mở Companion → Mắt nhìn AI → Bắt đầu phân tích ở trận luyện tập (chấp thuận gửi tối đa 45 ảnh tới Gemini). Sau ảnh đầu tiên, tạm chuyển sang cửa sổ Mắt nhìn AI để **xem ảnh cắt có đúng minimap** và ghi nhãn. Ảnh sai góc hoặc chứa nội dung riêng tư thì không lưu. Việc nhận diện tự động các biểu tượng Rừng/Mid, lần theo quỹ đạo đảo đường và đánh giá tình huống gank dựa trên chuỗi ảnh cần phát triển, huấn luyện và kiểm thử riêng; không coi văn bản tự do do Gemini tạo ra là kết quả nhận diện đã xác minh.
